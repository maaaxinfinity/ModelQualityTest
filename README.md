...
- Batch concurrency is scoped to one visible task category at a time. A category must finish before the next category starts, and an active run locks out switching to or starting work in another top-level model group.
