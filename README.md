...
- Every Image probe is fixed to `gpt-image-2` and runs in four ordered stages: Base64/URL return capability; real `/v1/images/edits` compositing with `1/2/4/8` input images; a `quality` (`low`/`medium`/`high`) × size matrix (`1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `2048x1152`, `3840x2160`, `2160x3840`, `auto`); then output `n=2/4/8` at 1K low quality. Edit inputs use a fixed target scene and seven distinct reference objects, render beside the result, and run serially for comparable latency. The server downloads each matrix URL (or decodes Base64), reads PNG/JPEG/WebP dimensions, and fails explicit-size cells unless returned pixels exactly match; `auto` is checked against image-2 constraints. The `n` probes run serially and render as a two-cell strip, 2×2 grid, and 3×3 grid with the center left empty. Image bytes are never persisted to PostgreSQL logs.
...
- The test page picks a target: select one or more endpoints, then pick one or more of each endpoint's **enabled models**. Every (question × endpoint × model) combination runs and renders side by side, so you can compare across endpoints and across models at once. The Image group is the exception: it exposes and runs only the exact `gpt-image-2` model.
