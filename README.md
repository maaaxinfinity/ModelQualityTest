...
- Image tests target `gpt-image-2` in three ordered stages: Base64/URL return capability, a true `quality` (`low`/`medium`/`high`) × documented-size matrix (`1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `2048x1152`, `3840x2160`, `2160x3840`, `auto`), then `n=2/4/8` at 1K low quality. The 24-cell matrix uses a restrained table UI rather than ordinary cards. The `n` probes run serially and render as a two-cell strip, 2×2 grid, and 3×3 grid with the center left empty. Returned images render directly in the browser; each run records total latency, and multi-image runs also show average latency per image. Base64 bytes are never persisted to PostgreSQL logs.
