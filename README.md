...
- Image tests target `gpt-image-2` in three ordered stages: Base64/URL return capability, a `quality` (`low`/`medium`/`high`) × `size` (`1024x1024`/`1536x1024`/`1024x1536`) matrix, then `n=1/2/4/8` at 1K low quality. The `n` probes run serially for comparable timing. Returned images render directly in the browser; each run records total latency, and multi-image runs also show average latency per image. Base64 bytes are never persisted to PostgreSQL logs.
